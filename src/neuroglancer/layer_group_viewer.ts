...
import {MouseSelectionState, SelectedLayerState} from 'neuroglancer/layer';
...
import {LinkedOrientationState, LinkedPosition, LinkedRenderDimensions, LinkedRenderScaleFactors, linkedStateLegacyJsonView, LinkedZoomState, NavigationState, Pose, TrackableCrossSectionZoom, TrackableNavigationLink, TrackableProjectionZoom} from 'neuroglancer/navigation_state';
...
export class LinkedViewerNavigationState extends RefCounted {
  ...
  renderScaleFactors: LinkedRenderScaleFactors;
  renderDimensions: LinkedRenderDimensions;
  ...
  constructor(parent: {
    ...
    super();
    this.renderScaleFactors = new LinkedRenderScaleFactors(
        parent.navigationState.pose.renderDimensions.renderScaleFactors.addRef());
    this.renderDimensions = new LinkedRenderDimensions(
        parent.navigationState.pose.renderDimensions.addRef(), this.renderScaleFactors.value);
    ...
    this.crossSectionScale = new LinkedZoomState(
        ...
        this.renderDimensions.value.addRef());
    this.navigationState = this.registerDisposer(new NavigationState(
        new Pose(
            this.position.value, this.renderDimensions.value, this.crossSectionOrientation.value),
    ...
    this.projectionNavigationState = this.registerDisposer(new NavigationState(
        new Pose(
            this.position.value.addRef(), this.renderDimensions.value.addRef(),
  ...
  register(state: CompoundTrackable) {
    state.add('dimensionRenderScales', this.renderScaleFactors);
    state.add('renderDimensions', this.renderDimensions);
...
function makeViewerMenu(parent: HTMLElement, viewer: LayerGroupViewer) {
  ...
  for (const [name, model] of <[string, TrackableNavigationLink][]>[
         ['Render scale factors', viewerNavigationState.renderScaleFactors.link],
         ['Render dimensions', viewerNavigationState.renderDimensions.link],
...
export class LayerGroupViewer extends RefCounted {
  ...
  private registerActionBindings() {
    ...
    this.bindAction('t-', () => {
      this.navigationState.pose.translateNonSpatialDimension(0, -1);
    });
    this.bindAction('t+', () => {
      this.navigationState.pose.translateNonSpatialDimension(0, +1);
    });
