...
export function createIdentity<T extends TypedArray, C extends TypedArrayConstructor<T>>(
    c: C, rows: number, cols: number = rows): T {
  return identity(new c(rows * cols), rows, Math.min(rows, cols));
}


export function
createHomogeneousScaleMatrix<T extends TypedArray, C extends TypedArrayConstructor<T>>(
    c: C, scales: ArrayLike<number>, square = true) {
 ...
 */
export function inverse<T extends TypedArray>(b: T, ldb: number, a: T, lda: number, n: number): number {
