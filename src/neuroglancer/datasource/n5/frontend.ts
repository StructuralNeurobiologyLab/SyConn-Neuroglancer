...
import { expectArray, parseArray, parseFixedLengthArray, verifyEnumString, verifyFinitePositiveFloat, verifyObject, verifyObjectProperty, verifyPositiveInt, verifyString, verifyOptionalObjectProperty} from 'neuroglancer/util/json';
...
export class MultiscaleVolumeChunkSource extends GenericMultiscaleVolumeChunkSource {
  ...
  constructor(
      public chunkManager: ChunkManager, public multiscaleMetadata: MultiscaleMetadata,
    ...
    this.modelSpace = makeCoordinateSpace({
      dimensionNames: baseModelSpace.dimensionNames,
      scales: baseModelSpace.scales,
      units: baseModelSpace.units,
      boundingBoxes: [
        {
          inputScales: baseModelSpace.scales,
          outputScales: baseModelSpace.scales,
    ...
    for (let i = 0; i < numScales; ++i) {
      ...
      sources.push(
          VolumeChunkSpecification
              .getDefaults({
                transform: createHomogeneousScaleMatrix(
                    Float32Array, scaleDownsamplingInfo.downsamplingFactor),
                dataType: scale.dataType,
                numChannels: 1,
              ...
              .map(spec => this.chunkManager.getChunkSource(N5VolumeChunkSource, {
                spec,
                parameters:
                    {'url': parseSpecialUrl(scaleDownsamplingInfo.url), 'encoding': scale.encoding}
              })));
    }
    return sources;
  }
