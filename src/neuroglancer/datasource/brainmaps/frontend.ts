...
export class MultiscaleVolumeInfo {
  ...
  getModelSpace(channelDimension: boolean = false): CoordinateSpace {
    ...
    if (channelDimension) {
      dimensionNames.push('c\'');
      units.push('');
      scales.push(1);
      lowerBounds.push(0);
      upperBounds.push(this.numChannels);
    }
    const scalesVec = Float64Array.from(scales);
    const outputScales = new Float64Array(dimensionNames.length);
    outputScales.fill(1);
    return makeCoordinateSpace({
      ...
      boundingBoxes: [{
        transform:
            matrix.createIdentity(Float64Array, dimensionNames.length, dimensionNames.length + 1),
        inputScales: scalesVec,
        outputScales,
        box: {
          lowerBounds: new Float64Array(dimensionNames.length),
...
export class MultiscaleVolumeChunkSource extends GenericMultiscaleVolumeChunkSource {
  ...
  get numChannels() {
    return this.channelDimension ? 1 : this.multiscaleVolumeInfo.numChannels;
  }
  ...
  constructor(
    ...
    this.channelDimension = options.channelDimension;
    this.modelSpace = this.multiscaleVolumeInfo.getModelSpace(this.channelDimension);
  ...
  getSources(volumeSourceOptions: VolumeSourceOptions) {
    ...
    return this.scales.map((volumeInfo, scaleIndex) => {
...
export class BrainmapsDataSource extends DataSourceProvider {
  ...
  get(options: GetDataSourceOptions): Promise<DataSource> {
    ...
    const channelDimension =
        verifyObjectProperty(parameters, 'channelDimension', x => x === 'true');
    const brainmapsOptions:
        GetBrainmapsVolumeOptions = {...options, encoding, chunkLayoutPreference, channelDimension};
    return options.chunkManager.memoize.getUncounted(
        ...
        () => Promise
                  ...
                  .then(([multiscaleVolumeInfo, meshesInfo]): DataSource => {
                    const volume = new MultiscaleVolumeChunkSource(
                        options.chunkManager, this.instance, this.credentialsProvider, volumeId,
                        changeSpec, multiscaleVolumeInfo, brainmapsOptions);
                    const dataSource: DataSource = {
                      modelSpace: volume.modelSpace,
                      resources: [{
                        id: meshName === undefined ? '' : 'volume',
                        url: undefined,  // FIXME
                        resource: {volume},
                    ...
                    const allMeshes =
                        getSingleScaleAndMultiscaleMeshes(multiscaleVolumeInfo, meshesInfo);
                    const addMeshResource = (mesh: MeshInfo, enabled: boolean) => {
                      ...
                      if (single !== undefined) {
                        if (single.type === 'TRIANGLES') {
                          meshSource = options.chunkManager.getChunkSource(BrainmapsMeshSource, {
                            credentialsProvider: this.credentialsProvider,
                            transform: getNanometersToVoxelsTransform(multiscaleVolumeInfo),
                            modelSpace: multiscaleVolumeInfo.getModelSpace(),
                            parameters: {
                              'instance': this.instance,
                              'volumeId': volumeId,
                          ...
                          meshSource =
                              options.chunkManager.getChunkSource(BrainmapsSkeletonSource, {
                                credentialsProvider: this.credentialsProvider,
                                transform: getNanometersToVoxelsTransform(multiscaleVolumeInfo),
                                modelSpace: multiscaleVolumeInfo.getModelSpace(),
                                parameters: {
                        ...
                        const {multi} = mesh;
                        meshSource =
                            options.chunkManager.getChunkSource(BrainmapsMultiscaleMeshSource, {
                              credentialsProvider: this.credentialsProvider,
                              format: {
                      ...
                      dataSource.resources.push({
                        id: meshName === undefined ? mesh.name : '',
                        url: undefined,  // FIXME
