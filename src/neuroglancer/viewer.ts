...
import {LayerManager, LayerSelectedValues, MouseSelectionState, SelectedLayerState} from 'neuroglancer/layer';
...
import {NavigationState, OrientationState, Pose, Position, TrackableCrossSectionZoom, TrackableProjectionZoom, TrackableRenderDimensions, TrackableRenderScaleFactors} from 'neuroglancer/navigation_state';
...
class TrackableViewerState extends CompoundTrackable {
  constructor(public viewer: Borrowed<Viewer>) {
    ...
    this.add('dimensionRenderScales', viewer.renderScaleFactors);
    this.add('renderDimensions', viewer.renderDimensions);
  ...
  restoreState(obj: any) {
    ...
    optionallyRestoreFromJsonMember(
        obj, 'perspectiveZoom', viewer.projectionScale.legacyJsonView);
    optionallyRestoreFromJsonMember(
...
export class Viewer extends RefCounted implements ViewerState {
  ...
  renderScaleFactors = this.registerDisposer(new TrackableRenderScaleFactors(this.coordinateSpace));
  renderDimensions =
      this.registerDisposer(new TrackableRenderDimensions(this.renderScaleFactors.addRef()));
  crossSectionOrientation = this.registerDisposer(new OrientationState());
  crossSectionScale = this.registerDisposer(new TrackableCrossSectionZoom(this.renderDimensions));
  projectionOrientation = this.registerDisposer(new OrientationState());
  projectionScale = this.registerDisposer(new TrackableProjectionZoom(this.renderDimensions));
  navigationState = this.registerDisposer(new NavigationState(
      new Pose(
          this.position.addRef(), this.renderDimensions.addRef(),
  ...
  constructor(public display: DisplayContext, options: Partial<ViewerOptions> = {}) {
    ...
    //     this.layerSpecification.spatialCoordinatesSet.add((spatialCoordinates: Float32Array) => {
    //       const {position} = this.navigationState;
    //       position.spatialCoordinates!.set(spatialCoordinates);
