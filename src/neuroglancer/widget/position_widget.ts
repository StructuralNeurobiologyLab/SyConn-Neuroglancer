...
import svg_iconCopy from 'ikonate/icons/copy.svg';
import { computeCombinedLowerUpperBound, CoordinateSpace, DimensionId, getDimensionNameValidity, insertDimensionAt, makeCoordinateSpace, validateDimensionNames, emptyCoordinateSpace} from 'neuroglancer/coordinate_transform';
...
class DimensionWidget {
  ...
  constructor(public coordinateSpace: CoordinateSpace) {
    ...
    nameContainer.title = 'Drag to reorder, double click to rename';
...
function getNormalizedDimensionBounds(
  ...
  const outputScale = coordinateSpace.scales[dimensionIndex];
  for (const boundingBox of boundingBoxes) {
    const result = computeCombinedLowerUpperBound(boundingBox, dimensionIndex, outputScale);
...
export class PositionWidget extends RefCounted {
  ...
  private openDropdown(widget: DimensionWidget) {
    ...
    const updateView = () => {
      ...
      if (hoverPosition !== undefined && hoverPosition >= lowerBound &&
        ...
        ctx.fillStyle = '#66f';
        const hoverOffset = Math.floor(hoverPosition * canvasHeight / (upperBound - lowerBound));
  ...
  private forceUpdateDimensions() {
    ...
    if (!coordinateSpace.valid) {
      coordinateSpace = emptyCoordinateSpace;
    }
    this.coordinateSpace = coordinateSpace;
    const {dimensionWidgets, dimensionWidgetList} = this;
    const {element} = this;
    dimensionWidgetList.length = 0;
    if (coordinateSpace !== undefined && coordinateSpace.rank !== 0) {
      const {
        dimensionNames,
        dimensionIds,
        scales,
      ...
      for (let i = 0; i < rank; ++i) {
        ...
        if (container !== nextChild) {
          element.insertBefore(container, nextChild);
        }
        nextChild = container.nextElementSibling;
      }
    }
    for (const [id, widget] of dimensionWidgets) {
      if (widget.coordinateSpace !== coordinateSpace) {
        try {
          element.removeChild(widget.container);
        } catch {
          // Ignore errors removing child
        }
        this.closeDropdown(widget);
  ...
  constructor(public position: Borrowed<Position>, {copyButton = true} = {}) {
    super();
    const {element} = this;
    ...
    if (copyButton) {
      ...
      copyButton.classList.add('neuroglancer-copy-button');
      copyButton.addEventListener('dragstart', event => {
        event.dataTransfer!.setData(positionDragType, JSON.stringify({
          position: position.toJSON(),
          dimensions: position.coordinateSpace.value.dimensionNames
        }));
  ...
  private updateNames() {
    ...
    const dimensionNames = dimensionWidgetList.map(x => x.nameElement.value);
    if (!validateDimensionNames(dimensionNames)) return false;
    const existingNames = existing.dimensionNames;
    if (arraysEqual(existingNames, dimensionNames)) return false;
    const dimensionTimestamps = existing.dimensionTimestamps.map(
        (t, i) => (existingNames[i] === dimensionNames[i]) ? t : Date.now());
    const newSpace = {...existing, dimensionNames, dimensionTimestamps};
  ...
  private updateScales() {
    ...
    const newSpace = makeCoordinateSpace({
      ...
      units: newUnits,
      dimensionTimestamps,
      dimensionIds: existing.dimensionIds,
      dimensionNames: existing.dimensionNames,
...
export class MousePositionWidget extends RefCounted {
  ...
  updateView() {
    ...
    if (mouseState.active && coordinateSpace !== undefined) {
      ...
      for (let i = 0; i < rank; ++i) {
        if (i !== 0) text += '  ';
        text += `${dimensionNames[i]} ${Math.floor(p[i])}`;
      }
    }
