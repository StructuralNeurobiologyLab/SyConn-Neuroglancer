...
export class MultipleScaleBarTextures extends RefCounted {
  ...
  draw(
      viewportWidth: number, renderDimensions: RenderDimensions, effectiveZoom: number,
    options: ScaleBarOptions) {
    const {scaleBars} = this;
    const {
      rank,
      dimensionIndices,
      renderScaleFactors: {factors, coordinateSpace: {dimensionNames, scales, units}},
    ...
    for (let i = 0; i < rank; ++i) {
      ...
      scaleBar!.label += `${dimensionNames[dim]} `;
    }
