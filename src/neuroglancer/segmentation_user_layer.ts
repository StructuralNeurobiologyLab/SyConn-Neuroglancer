...
import {LayerDataSourceSpecification, layerDataSourceSpecificationFromJson, LoadedLayerDataSource} from 'neuroglancer/layer_data_source';
...
import {FRAGMENT_MAIN_START as SKELETON_FRAGMENT_MAIN_START, PerspectiveViewSkeletonLayer, SkeletonLayer, SkeletonRenderingOptions, SliceViewPanelSkeletonLayer, ViewSpecificSkeletonRenderingOptions} from 'neuroglancer/skeleton/frontend';
...
export class SegmentationUserLayer extends Base {
  ...
  initializeDataSource(loaded: LoadedLayerDataSource, refCounted: RefCounted) {
    for (const loadedSubresource of loaded.enabledSubresources) {
      const {volume, mesh} = loadedSubresource.subresource.resource;
      if (volume instanceof MultiscaleVolumeChunkSource) {
        const renderLayer = new SegmentationRenderLayer(volume, {
          ...this.displayState,
        ...
        const displayState = {
          ...
          transform: refCounted.registerDisposer(getWatchableRenderLayerTransform(
              this.manager.root.coordinateSpace, this.localPosition.coordinateSpace,
              mesh.modelSpace, loaded.transform)),
        };
        if (mesh instanceof MeshSource) {
          const renderLayer = new MeshLayer(this.manager.chunkManager, mesh, displayState);
          refCounted.registerDisposer(this.addRenderLayer(renderLayer));
          ...
          {
            const renderLayer = new PerspectiveViewSkeletonLayer(base.addRef());
            refCounted.registerDisposer(this.addRenderLayer(renderLayer));
            refCounted.registerDisposer(loadedSubresource.messages.addChild(renderLayer.messages));
          }
          {
            const renderLayer = new SliceViewPanelSkeletonLayer(/* transfer ownership */ base);
            refCounted.registerDisposer(this.addRenderLayer(renderLayer));
            refCounted.registerDisposer(loadedSubresource.messages.addChild(renderLayer.messages));
          }
        }
      } else {
        loadedSubresource.messages.addMessage(
            {severity: MessageSeverity.error, message: 'Not compatible with segmentation layer'});
      }
    }
  }
  ...
  getLegacyDataSourceSpecifications(
    ...
    if (skeletonsPath != null) {
      specs.push(layerDataSourceSpecificationFromJson(
          this.manager.dataSourceProviderRegistry.convertLegacyUrl({url: skeletonsPath, type: 'skeletons'})));
    }
    return specs;
  }
