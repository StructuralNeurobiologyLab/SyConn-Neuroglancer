...
import {makeRenderedPanelVisibleLayerTracker} from 'neuroglancer/layer';
...
export class SliceViewPanel extends RenderedDataPanel {
  ...
  translateByViewportPixels(deltaX: number, deltaY: number): void {
    ...
    pose.updateSpatialPosition3d(pos => {
  ...
  isReady() {
    ...
    for (const renderLayer of this.visibleLayerTracker.visibleLayers.keys()) {
  ...
  drawWithPicking(pickingData: FramePickingData): boolean {
    ...
    const {
      navigationState:
          {pose: {renderDimensions: {value: renderDimensions}, position: {value: globalPosition}}}
    } = this;
    ...
    const drawLayer = (renderLayer: SliceViewPanelRenderLayer, messages: MessageList) => {
      try {
        renderLayer.draw(renderContext);
      } catch (e) {
        messages.addMessage({severity: MessageSeverity.error, message: (e as Error).message});
      }
    };
    for (const [renderLayer, {messages}] of visibleLayers) {
      messages.clearMessages();
      drawLayer(renderLayer, messages);
    }
    ...
    if (this.viewer.showAxisLines.value || this.viewer.showScaleBar.value) {
      if (this.viewer.showAxisLines.value) {
        ...
        const {voxelPhysicalScales: globalScales} = sliceView.globalTransform!;
        const spatialRank = globalScales.length;
        for (let j = 0; j < spatialRank; ++j) {
          for (let i = 0; i < 2; ++i) {
      ...
      if (this.viewer.showScaleBar.value) {
        ...
        gl.blendFunc(WebGL2RenderingContext.SRC_ALPHA, WebGL2RenderingContext.ONE_MINUS_SRC_ALPHA);
        this.scaleBars.draw(
            this.width, this.navigationState.pose.renderDimensions.value,
  ...
  completePickRequest(
    ...
    const {rank: displayRank, dimensionIndices} = this.navigationState.pose.renderDimensions.value;
  ...
  zoomByMouse(factor: number) {
    ...
    const {dimensionIndices, rank: displayRank} = navigationState.pose.renderDimensions.value;
