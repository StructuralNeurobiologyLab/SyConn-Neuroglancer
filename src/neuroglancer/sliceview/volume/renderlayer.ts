...
`);
  ...
  computeVerticesDebug(
      uChunkDataSize: vec3, uLowerClipBound: vec3, uUpperClipBound: vec3,
    ...
    for (let vertexIndex = 0; vertexIndex < 6; ++vertexIndex) {
      const position = this.computeVertexPositionDebug(
          uChunkDataSize, uLowerClipBound, uUpperClipBound, uPlaneDistance, uPlaneNormal, uTranslation,
          vertexIndex);
  ...
  beginSource(
    ...
    this.setViewportPlane(
        shader, sliceView.viewportNormalInGlobalCoordinates, sliceView.centerDataPosition,
    ...
    gl.uniform3fv(shader.uniform('uLowerClipBound'), tsource.lowerClipSpatialBound);
    gl.uniform3fv(shader.uniform('uUpperClipBound'), tsource.upperClipSpatialBound);
    if (DEBUG_VERTICES) {
      (<any>window)['debug_sliceView_uLowerClipBound'] = tsource.lowerClipSpatialBound;
      (<any>window)['debug_sliceView_uUpperClipBound'] = tsource.upperClipSpatialBound;
      (<any>window)['debug_sliceView'] = sliceView;
      (<any>window)['debug_sliceView_dataToDevice'] = mat4.clone(tempMat4);
      (<any>window)['debug_sliceView_chunkLayout'] = tsource.chunkLayout;
    }
  }
...
export abstract class RenderLayer<ShaderParameters = any> extends SliceViewRenderLayer {
  ...
  private tempPosition: Float32Array;
  shaderParameters: WatchableValueInterface<ShaderParameters>;
  constructor(
      multiscaleSource: MultiscaleVolumeChunkSource,
...
`);
    ...
    this.tempPosition = new Float32Array(multiscaleSource.modelSpace.dimensionNames.length);
  }
  ...
  getValueAt(globalPosition: Float32Array) {
    ...
    const {globalToRenderLayerDimensions, localToRenderLayerDimensions} = transform;
    const localPosition = this.localPosition.value;
    /// Need to convert from global space to layer space.
    ...
    for (let globalDim = 0; globalDim < globalRank; ++globalDim) {
      ...
      tempPosition[layerDim] = globalPosition[globalDim];
    }

    for (let localDim = 0; localDim < localRank; ++localDim) {
      const layerDim = localToRenderLayerDimensions[localDim];
      if (layerDim === -1) continue;
      tempPosition[layerDim] = localPosition[localDim];
    }

    for (const info of this.visibleSourcesList) {
      const source = info.source as VolumeChunkSource;
      const inverseTransform = this.getInverseTransform(info);
      let result = source.getValueAt(tempPosition, inverseTransform!);
      if (result != null) {
        return result;
      }
    }
  ...
  draw(sliceView: SliceView) {
    ...
    const chunkSpatialDataSize = vec3.create();
    ...
    for (const transformedSource of visibleSources) {
      ...
      const {fixedPositionWithinChunk, spatialChunkDimensions} = transformedSource;
      for (const chunkDim of spatialChunkDimensions) {
      ...
      vertexComputationManager.beginSource(
          gl, shader, sliceView, sliceView.viewProjectionMat, transformedSource);
        ...
        if (chunk && chunk.state === ChunkState.GPU_MEMORY) {
          ...
          if (newChunkDataSize !== chunkDataSize) {
            ...
            for (let i = 0; i < 3; ++i) {
              ...
              chunkSpatialDataSize[i] = chunkDim === -1 ? 1 : chunkDataSize[chunkDim];
            }
            vertexComputationManager.setupChunkDataSize(gl, shader!, chunkSpatialDataSize);
          }
          const {chunkGridPosition} = chunk;
          for (let i = 0; i < 3; ++i) {
            const chunkDim = spatialChunkDimensions[i];
            chunkPosition[i] =
                chunkDim === -1 ? 0 : originalChunkSize[i] * chunkGridPosition[chunkDim];
          }
          chunkFormat.bindChunk(
              gl, shader, chunk, fixedPositionWithinChunk, spatialChunkDimensions, newSource);
          newSource = false;
